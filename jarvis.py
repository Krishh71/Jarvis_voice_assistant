...
def process_command(command):
    ...
    if "open google" in command:
        ...
        if len(parts) > 1:
            song = parts[1]
            if hasattr(musiclib, 'music') and song in musiclib.music:
...
def listen_for_wake_word():
    with spr.Microphone() as source:
        ...
        try:
            # Adjusting for ambient noise helps accuracy in VS Code
...
def active_mode():
    speak("Yes, I am listening. Say Jarvis off to stop.")

    while True:
        with spr.Microphone() as source:
            print("Jarvis is listening for a command...")
            try:
                audio = recognizer.listen(source)
                command = recognizer.recognize_google(audio).lower()
                print("Command:", command)

                if "jarvis off" in command or "stop listening" in command:
